...
function SidebarSubmenu({ route }) {
  ...
  return (
    <li id="list-item" className="relative px-6 py-3" key={route.name}>
      <button
      ...
      >
        <ul
          ...
          aria-label="submenu"
          style={{ backgroundColor: "var(--dash-nav)", textAlign: "right" }}
        >
          {route.routes.map((r) => (
            <li
            ...
            >
              <Link
                className="w-full ms-3 text-decoration-none text-secondary"
                to={r.path}
              >
                {r.name}
              </Link>
