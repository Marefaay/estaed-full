...
const EditSchool = () => {
    ...
    const getSchool = () => {
      fetch(`${url}/${id}`)
        ...
        .then((data) => {
          ...
          setStudents(data.studentsNumber);
          setCity(data.cityName);
          setManagemt(data.managmentNumber);
          setTime(data.time);
          setEmail(data.mail);
  ...
  return (
    <div>
      <PageTitle>تعديل المدرسة</PageTitle>

      {/* Breadcum */}
      <div className="d-flex  align-items-center text-gray-800  dark:text-gray-300">
        <div className="d-flex align-items-center link" >
          <Icon className="w-5 h-5" aria-hidden="true" icon={HomeIcon} />
          <NavLink exact to="/dashboard" className="mx-2 text-decoration-none" style={{color:"#ff4400"}}>
            لوحةالتحكم
          </NavLink>
        </div>
       <span style={{color:"var(--text-color)"}}> {">"}</span>
        <div className="flex items-center link">
          {/* <Icon className="w-5 h-5" aria-hidden="true" icon={HomeIcon} /> */}
          <NavLink exact to="/all-schools" className="mx-2 text-decoration-none" style={{color:"#ff4400"}}>
            كل المدارس
          </NavLink>
        </div>
       <span style={{color:"var(--text-color)"}}> {">"}</span>
        {/* {">"} */}
        <p className="mx-2 pt-3" style={{color:"var(--text-color)"}}>{JSON.stringify(data.schoolName)}</p>
      </div>
      ...
      <div className="w-100   ">
        <Card className="bg-transparent" id="card-body">
          <CardBody id="form">
            ...
            {schoolError != null ? (
              <div
                className="d-flex justify-content-between w-75 ms-3"
                style={{ color: "red" }}
              >
            ...
            <div className="w-100 text-center d-flex">
              ...
              <button id="add-school-btn" className=" text-center" onClick={handleFormSubmit}>
                تعديل المدرسة
              </button>
            </div>
          </CardBody>
